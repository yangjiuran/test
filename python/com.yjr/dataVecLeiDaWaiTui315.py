...
## 本地存储参数
#datapath = r'/home/ef/dev/py_script/data_new'      # txt文件下载路径
...
#imagespath = r'/home/ef/dev/py_script/images'  # 图片存储路径
...
def genarateSHPfile_zs(pintu, name,LLevels):
    cm_light = mpl.colors.ListedColormap(colormap)#设置colormap实例
    Levels = [0,5,10,15,20,25,30,35,40,45,50,55,60,65,70]
    nLevel = len(Levels)
    cs = plt.contour(lon_grid, lat_grid, pintu, Levels, alpha = 1, cmap=cm_light)
    ...
    for ind in range(1, nLevel):
        ...
        for i in range(len(temp)):
            if len(temp[i].vertices[::-1].tolist())>=3:
                level.append(temp[i].vertices[::-1].tolist())
        #自定义选择等级添加数据
        if nLevel-ind in LLevels:
            levels.append(level)
            records.append(nLevel-ind)
    # 生成shp文件
    with shp.Writer(name) as w:
        w.autoBalance = 1
        w.field('Data', 'C')
...
if __name__ == "__main__":
    ...
    LLevels = (sys.argv[2]).split(':')
    LLevels = list(range(int(LLevels[0]),int(LLevels[-1])+1))
    ...
    for datafile in file_lists:
        ...
        if type == '.txt':
            ...
            maxlevel_zs = genarateSHPfile_zs(pintu,shpname_zs,LLevels)
